...
(function (root, factory) {
  ...
  // updates section dimensions upon resize (to avoid calling expensive
  // getBoundingClientRect() on each scrolling act)
      ...
      if (!entry) {
        ...
        scroller.vpl = (function (entry) {
          return function () {
            ...
            var rateVert = maxVert
              ? scroller[scroll + Top] / maxVert
              : 0
            var rateHoriz = maxHoriz
              ? scroller[scroll + Left] / maxHoriz
              : 0

            // viewport location point moves along with viewport scroll to always meet the
            // borders
            ...
            // updating the data for each section (and searching for the closest section)
            var closest = _null
            var minDist = _null
            ...
            for (var i = 0; i < entry.s[length];) {
              ...
              var vVertDist = Math_max(0, Math_abs(vTopLocation - 0.5) - 0.5)
              var vHorizDist = Math_max(0, Math_abs(vLeftLocation - 0.5) - 0.5)

              // squared, but we only need to compare
              var dist = vVertDist * vVertDist + vHorizDist * vHorizDist

              var scrollTopToStart = -topOffset - ctx
              var scrollTopToMiddle = (sBottom + sTop) / 2 - vMiddle

              var scrollLeftToStart = -leftOffset - ctx
              var scrollLeftToCenter = (sLeft + sRight) / 2 - vCenter
              ...
              section[VIEWPORT + Scroll + Top + Target] = Math_max(0, Math_min(maxVert, scroller[scroll + Top] + Math_min(scrollTopToStart, scrollTopToMiddle)))

              section[VIEWPORT + Scroll + Left + Target] = Math_max(0, Math_min(maxHoriz, scroller[scroll + Left] + Math_min(scrollLeftToStart, scrollLeftToCenter)))
